...
# guardar los logaritmos
log_price_train <- train$log_price
log_price_test <- test$log_price
...
keep <- c("property_typeCasa", "police", "parking" ,"cinema_d", "Final_Bathrooms_2",
          "mod_VA1_ESTRATO26", "cinema","marketplace", "kindergarten" , "mod_VA1_ESTRATO23","log_price")

train <- train[, (names(train) %in% keep)]
test <- test[, (names(test) %in% keep)]

# crear matriz de dummies con las variables categoricas
df <- model.matrix(~(.)^2 -1, train) %>%data.frame()

write_csv(df,"data/df_dummies_en.csv")


# guardar las variables de interacciones sin el log_price

drop_log_inter <- c("Final_Bathrooms_2.log_price","cinema_d.log_price", "cinema.log_price",
                    "kindergarten.log_price","marketplace.log_price","parking.log_price", "police.log_price" )


df <- df[, (names(df) %in% keep_nolog)]
